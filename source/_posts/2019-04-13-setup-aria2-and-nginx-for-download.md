...
mkdir .aria2
cd .aria2/
touch aria2.conf aria2.session dht6.dat dht.dat
```
...
```
 ...
 启用并运行： `systemctl enable aria2.service && systemctl start aria2.service`
...
- 然后 `cd /etc/nginx/sites-enabled/` ，创建配置文件的软链接：
...
![webui-aria2](2019-04-13-setup-aria2-and-nginx-for-download/webui-aria2.jpg)
